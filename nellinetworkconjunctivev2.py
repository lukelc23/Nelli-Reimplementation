### 
# Changes:
# - Layers 3 and 4 have their own initializations
...
class Network(torch.nn.Module):
    def __init__(self, items_n, h1_size, w1_weight_std, w2_weight_std, w3_weight_std, w4_weight_std, non_linearity=torch.relu_, readouts=1):
        ...
        Additionally, the network has a conjunctive layer which adds functionality to the above additive network.
        ...
        self.layer_3 = torch.nn.Linear(items_n * 2, h1_size, bias=False)
        self.layer_4 = torch.nn.Linear(h1_size, readouts, bias=False)
        torch.nn.init.normal_(self.layer_1.weight, std=w1_weight_std)
        torch.nn.init.normal_(self.layer_2.weight, std=w2_weight_std)
        # layer 3/4 initialized to have their own initializations
        torch.nn.init.normal_(self.layer_3.weight, std=w3_weight_std)
        torch.nn.init.normal_(self.layer_4.weight, std=w4_weight_std)
